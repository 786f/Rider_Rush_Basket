...
class OrderData {
  ...
  final String status;
  final double total;
  final double deliveryAmount; // ✅ FIXED
  final String paymentMethod;
  final String userPhone;
  final Address shippingAddress;
  final Vendor vendor;
  ...
  factory OrderData.fromJson(Map<String, dynamic> json) {
    ...
    return OrderData(
      ...
      orderNumber: json['orderNumber'] ?? '',
      status: json['status'] ?? '',
      total: (json['pricing']?['total'] as num? ?? 0).toDouble(), // ✅ SAFE
      deliveryAmount:
      (json['pricing']?['deliveryAmount'] as num? ?? 0).toDouble(), // ✅ FIXED
      paymentMethod: json['payment']?['method'] ?? '',
      userPhone: json['user']?['contactNumber'] ?? '',
      shippingAddress:
      Address.fromJson(json['shippingAddress'] ?? {}),
...
class OrderItem {
  ...
  factory OrderItem.fromJson(Map<String, dynamic> json) {
    return OrderItem(
      productName: json['productName'] ?? '',
      quantity: json['quantity'] ?? 0,
      price: (json['totalPrice'] as num? ?? 0).toDouble(), // ✅ SAFE
      image: json['image']?['url'] ??
          json['thumbnail']?['url'] ??
          '',
    );
...
class Address {
  ...
  factory Address.fromJson(Map<String, dynamic> json) {
    return Address(
      ...
      pinCode: json['pinCode'] ?? '',
      latitude:
      (json['latitude'] as num? ?? 0).toDouble(), // ✅ FIXED
      longitude:
      (json['longitude'] as num? ?? 0).toDouble(), // ✅ FIXED
    );
  }
}
...
class Vendor {
  ...
  factory Vendor.fromJson(Map<String, dynamic> json) {
    return Vendor(
      ...
      vendorName: json['vendorName'] ?? '',
      storeAddress:
      VendorAddress.fromJson(json['storeAddress'] ?? {}),
    );
  }
